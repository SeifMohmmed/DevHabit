    ...
    : FunctionalTestFixture(appFactory), IAsyncLifetime
{
    ...
    public async Task CompleteHabitEntryFlow_ShouldSucceed()
    {
        ...
        AccessTokenDto? accessTokens = await loginResponse.Content.ReadFromJsonAsync<AccessTokenDto>();
        ...
        HttpResponseMessage getEntriesResponse = await client.GetAsync(
            new Uri($"{Routes.EntryRoutes.GetAll}?habitId={createdHabit.Id}", UriKind.Relative));
